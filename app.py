...
def route_query(query: str) -> str:
    ...
    wants_market    = any(w in q for w in [
        "market", "index", "indices", "dow", "nasdaq", "s&p"
    ])
    ...
    # merge with labels
    out, idx = [], 0
    if wants_price:     out.append("**Price:** "      + responses[idx]); idx += 1
    if wants_news:      out.append("**News:** "       + responses[idx]); idx += 1
    if wants_financial: out.append("**Financials:** " + responses[idx]); idx += 1
    if wants_market and not wants_news:
        out.append("**Market:** " + responses[idx]); idx += 1
    return "\n\n".join(out)

# ─── Streamlit page config ───────────────────────────────────────────────────
st.set_page_config(page_title="💹 Multi-Agent Stock Chatbot",
                   page_icon="💹",
                   layout="wide")
...
with st.sidebar:
    ...
    # Logged-in footer
    display_name = (
        auth.get_full_name(st.session_state.username)
...
query_text = st.text_input("Your question:",
                           key="query_input",
                           placeholder="e.g. What’s the outlook for AAPL earnings?")
